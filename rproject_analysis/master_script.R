...
bpml <- read.tree(file = "bptree") ### file read (alternative)
tpml <- read.tree(file = "tptree") ### file read (alternative)
...
bpmldata <- select(pcabp$fac, FN, Site, Country, ATU, ATU2, NAT) %>% droplevels()
tpmldata <- select(pcatp$fac, FN, Site, Country, ATU, ATU2, NAT) %>% droplevels()
